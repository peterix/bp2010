...
Další postup by měl spíše směřovat k~obnovení funkčnosti jednotlivých omezení akcí a zdrojů dat v~Kiosku. Kiosk jako celek je bohužel jednou z~méně udržovaných částí KDE-Libs a~určitá část nastavitelných omezení je zcela neefektivní. Ne všechny jeho části přežily přechod z~KDE3 na KDE4 bez úhony. Možným prvním krokem by mohlo být odstranění zdvojení funkce pro autorizaci akcí (\cppc{KAuthorized::authorize()} a~\cppc{KAuthorized::authorizeKAction()}), které vede někdy k jejich zaměňování.
...
\item[polkitd] je systémová služba PolicyKitu a~implementuje tzv. PolicyKit autoritu.
